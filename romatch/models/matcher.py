...
class RegressionMatcher(nn.Module):
    ...
    def match(
        ...
        *args,
        ...
        """
        Находит плотные соответствия между двумя изображениями.

        Эта функция обрабатывает входные изображения, 
        пропускает их через модель мэтчинга для получения поля потока и карты уверенности,
        а затем форматирует их в виде поля деформации (warp field) и карты уверенности.

        Args:
            im_A_input (str | os.PathLike | Image.Image | torch.Tensor):
                Первое изображение. Может быть путем к файлу, объектом PIL.Image или тензором PyTorch.
            im_B_input (str | os.PathLike | Image.Image | torch.Tensor):
                Второе изображение. Может быть путем к файлу, объектом PIL.Image или тензором PyTorch.
            batched (bool, optional): Если True, предполагает, что входные данные являются батчами изображений.
                В этом режиме изображения должны иметь одинаковый размер. По умолчанию False.
            device (torch.device, optional): Устройство для выполнения вычислений. Если None,
                автоматически выбирается CUDA, если доступно, иначе CPU. По умолчанию None.

        Returns:
            tuple[torch.Tensor, torch.Tensor]: Кортеж, содержащий поле деформации и карту уверенности.

            - `warp` (torch.Tensor): Тензор, представляющий плотное поле соответствий.
              Координаты нормализованы в диапазоне [-1, 1]. Его форма зависит от `batched` и `self.symmetric`:
                - Если `batched=False` и `self.symmetric=False`: форма `(H, W, 4)`.
                  Для каждой точки `(y, x)` в изображении A, `warp[y, x]` содержит `(x_A, y_A, x_B, y_B)`,
                  где `(x_A, y_A)` - координаты в изображении A, а `(x_B, y_B)` - соответствующие координаты в изображении B.
                - Если `batched=False` и `self.symmetric=True`: форма `(H, 2*W, 4)`. Левая половина
                  `warp[:, :W, :]` представляет собой деформацию из A в B, а правая половина `warp[:, W:, :]` - из B в A.
                - Если `batched=True`: добавляется измерение батча в начале, например, `(B, H, W, 4)`.

            - `certainty` (torch.Tensor): Тензор, представляющий уверенность модели в каждом соответствии.
              Значения находятся в диапазоне [0, 1]. Его форма соответствует пространственным размерам `warp`:
                - Если `batched=False` и `self.symmetric=False`: форма `(H, W)`.
                - Если `batched=False` и `self.symmetric=True`: форма `(H, 2*W)`.
                - Если `batched=True`: добавляется измерение батча, например, `(B, H, W)`.
        """
        if device is None:
